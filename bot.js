...
async function handleFAQ(chatId) {
  ...
  const faqText = `<b>✳️ Куда обращаться, если что-то случилось с подпиской?</b>\n\n
...
<b>✳️ Как происходит подключение?</b>  \n\n
После оплаты вы должны отправить логин и пароль.\n\n\n
<b>✳️ Что делать, если подписку хочу, но за последний год уже находился в семейном плане?</b>\n\n
...
Не дольше получаса, зависит от загруза сервиса, обычно 5-15 минут`;
  ...
  // Пример отправки (замените 'bot' на вашу переменную бота, например, из node-telegram-bot-api)
  await bot.sendPhoto(chatId, faqImage, {
    caption: faqText,
    parse_mode: 'HTML'  // Это ключевой параметр для поддержки форматирования!
  });
  // Если без фото, используйте bot.sendMessage(chatId, faqText, { parse_mode: 'HTML' });
}
